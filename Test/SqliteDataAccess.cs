...
namespace Bookmarks
{
    public class SqliteDataAccess
    {
        ...
        public async static void SaveBookmark(string link, string title) {
            using (IDbConnection cnn = new SQLiteConnection(LoadConnectionString()))
            {
                WebClient x = new WebClient();
                await cnn.ExecuteAsync("insert into Bookmarks (Link, Title) values (@link, @title)", new { @link = link, @title = title});
            }
        }
        ...
        public static List<int> LoadFoldersFromParentID(int parent_id)
        {
            using (IDbConnection cnn = new SQLiteConnection(LoadConnectionString()))
            {
                var output = cnn.Query<int>("select ID from Folders where InFolderID = @parent_id", new { @parent_id = parent_id });
                return output.ToList();
            }
        }
        ...
        public async static Task<int> LoadFolderParentFromID(int ID)
        {
            using (IDbConnection cnn = new SQLiteConnection(LoadConnectionString()))
            {
                return await cnn.QueryFirstAsync<int>(@"select InFolderID from Folders where ID = @_ID", new { @_ID = ID });
            }
        }

        public static void UpdateFolder(int ID, string name)
        {
            using (IDbConnection cnn = new SQLiteConnection(LoadConnectionString()))
            {
        ...
        public async static void SaveFolder(string name)
        {
            using (IDbConnection cnn = new SQLiteConnection(LoadConnectionString()))
            {
                WebClient x = new WebClient();
                await cnn.ExecuteAsync("insert into Folders (Name) values (@name)", new { @name = name });
            }
        }
